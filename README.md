...
При запуске после сбоя, приложение продолжит добавление транзакций в базу данных, проверяя те, которые уже есть в базе данных и обновит значения неключевых полей, если они были изменены другими пользователями. Сделал как легче так как не смог предположить какой результат ожидается.
...
java -jar относительный_или_абсолютный_путь_к_файлу_ukrsib-1.0.jar
