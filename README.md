...
Тестовое задание Укрсиббанк (предположительно)
...
1.3. Другие пользователи могут менять неключевые поля во время работы приложения.
...
Приложение не будет обновлять значения неключевых полей Клиентов, если они были изменены другими пользователями (так как включен кэш 2-го уровня). Исходя из предположения 1.3.

При запуске после сбоя, приложение продолжит добавление транзакций в базу данных, проверяя те, которые уже есть в базе данных и обновит значения неключевых полей, если они были изменены другими пользователями. Сделал как легче.
...
Так же, для тестов используются файлы Java_test_3000.xml и Java_test_nulls.xml
...
6.	Maven
-------------------------------------------------
Сборка приложения со всеми зависимостями и без:

mvn package

Сборка приложения со всем зависимостями и запуск:

mvn package spring-boot:run

или после сборки запустить jar в папке target
java -jar относительный_или_абсолютный_путь_к_файлу_target/ukrsib-1.0.jar
...
7. Проверено
------------------------------------------
Сборка Maven и работа приложения проверена на:

Windows Server 2008 R2, MySQL 5.7, java 12/14

Windows 10, MySQL 5.7, java 12/14

Ubuntu 20, MySQL 8, java 14
